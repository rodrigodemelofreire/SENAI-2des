...
function preecherTabela() {
   dados.forEach((e, i) => {
      ...
      detalhes.innerHTML = `<button id="butaozin" onclick="abrirModal(${i})">Detalhes</button>`;
...
function abrirModal(i) {
   detalhes.classList.toggle("oculto");
   document.querySelector("#id").textContent = dados[i].id;
   document.querySelector("#nome").textContent = dados[i].nome;
   document.querySelector("#descricao").textContent = dados[i].descricao;
   document.querySelector("#preco").textContent = dados[i].preco;
}
